...
const {keys, getPrototypeOf, prototype: protoObject} = Object
const {slice: _slice, reduce} = Array.prototype
...
pub.call = call
function call (fun) {
  arguments[0] = undefined  // this relies on strict mode
...
const pipe = pub.pipe = rest(function pipe (funs) {
  ...
  return !funs.length
    ? id
    : function () {return foldl(stepPipe, arguments, funs)}
})
...
function foldl (fun, acc, list) {
  list = toList(list)
  for (let i = -1; ++i < list.length;) acc = fun(acc, list[i], i)
  return acc
}
...
function foldr (fun, acc, list) {
  list = toList(list)
  for (let i = list.length; --i >= 0;) acc = fun(acc, list[i], i)
  return acc
}
...
function map (fun, list) {
  list = toList(list)
  const out = []
  for (let i = -1; ++i < list.length;) out.push(fun(list[i], i))
  return out
}
...
function filter (test, list) {
  list = toList(list)
  const out = []
  for (let i = -1; ++i < list.length;) if (test(list[i], i)) out.push(list[i])
  return out
}
...
function find (test, list) {
  list = toList(list)
  for (let i = -1; ++i < list.length;) if (test(list[i], i)) return list[i]
}
...
function mask (pattern) {
  return isFunction(pattern)
    ...
    : isRegExp(pattern)
    ? pattern.test.bind(pattern)
    : objectMask(mapVals(mask, pattern))
}
...
function applyMask (pattern, object) {
  return mapVals((mask, key) => mask(object[key]), pattern)
}
