...
function not (fun) {
  validate(isFunction, fun)
  return function () {return !fun(...arguments)}
}
...
function cond (test, fun) {
  validateEach(isFunction, arguments)
  return fun
    ? ifelse(test, fun, cond(...slice(arguments, 2)))
    : test || noop
}
...
function stepPipe (value, fun, index) {
  return !index ? fun(...value) : fun(value)
}
...
function pipeAnd (fun) {
  ...
  return pipe(fun || id, funs.reduce.bind(funs, stepPipeAnd))
}
...
pub.juxt = rest(function juxt (funs) {
  validateEach(isFunction, funs)
  return function () {
    return funs.map(fun => fun(...arguments))
  }
})
...
function test (pattern) {
  return isFunction(pattern)
    ...
    : isList(pattern)
    ? bind(testList, map(test, pattern))
    : dictTest(pattern)
}

function dictTest (pattern) {
  return and(...prepend(mapDict(keyTest, pattern), isObject))
}
...
function testList (tests, list) {
  return isList(list) && tests.every(testOne, list)
}

function testOne (test, i) {
  return test(this[i])
}
...
function mask (pattern) {
  return isFunction(pattern)
    ...
    ? pattern.test.bind(pattern)
    : isList(pattern)
    ? pipe(toList, bind(compoundMask, map(mask, pattern), map))
    : pipe(Object, bind(compoundMask, mapVals(mask, pattern), mapVals))
}

function compoundMask (pattern, map, value) {
  return map((mask, key) => mask(value[key]), pattern)
}
