...
pub.cond = cond
function cond (test, fun, ...rest) {
  if (!arguments.length) return noop
  validate(isFunction, test)
  if (arguments.length === 1) return test
  return ifelse(test, fun, cond(...rest))
}

pub.pipe = pipe
function pipe (fun, ...funs) {
  if (!arguments.length) return id
  validate(isFunction, fun)
  if (arguments.length === 1) return fun
  validateEach(isFunction, funs)
  return function () {return funs.reduce(callRight, fun(...arguments))}
}
